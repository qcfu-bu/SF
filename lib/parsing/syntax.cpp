...
std::pair<std::vector<std::string>, std::vector<Name::Seg>> Name::slice() const {
    std::vector<std::string> path;
    std::vector<Seg> rest;
    for (auto it = this->path.begin(); it != this->path.end(); ++it) {
        if (const auto* ptr = std::get_if<std::string>(&(*it))) {
            path.push_back(*ptr);
        } else {
            rest.insert(rest.end(), it, this->path.end());
            break;
        }
    }
    return { path, rest };
...
// Format Name
static std::string format_name(const Name& name) {
...
std::string format_type(const Type& type) {
    switch (type.get_kind()) {
        ...
        case Type::Kind::Unit:
            return "()";
        case Type::Kind::Name: {
            const auto& t = static_cast<const NameType&>(type);
            return format_name(t.name) + format_type_args(t.type_args);
        }
...
std::string format_pat(const Pat& pat) {
    switch (pat.get_kind()) {
        ...
        case Pat::Kind::Ctor: {
            ...
            std::string result = format_name(p.name) + format_type_args(p.type_args);
            if (!p.args.has_value()) {
                return result;
            }
            result += "(";
        ...
        case Pat::Kind::Name: {
            const auto& p = static_cast<const NamePat&>(pat);
            std::string result;
            if (p.is_mut) {
                result += "mut ";
            }
            result += "%" + format_name(p.name);
    ...
    switch (expr.get_kind()) {
        ...
        case Expr::Kind::Unary: {
            ...
            switch (u.get_op()) {
                ...
                case UnaryExpr::Op::Dot: {
                    const auto& e = static_cast<const DotExpr&>(expr);
        ...
        case Expr::Kind::Name: {
            ...
            return format_name(e.name) + format_type_args(e.type_args);
        }
...
std::string format_stmt(const Stmt& stmt, int indent) {
    ...
    switch (stmt.get_kind()) {
        case Stmt::Kind::Open: {
            const auto& s = static_cast<const OpenStmt&>(stmt);
            result += "open " + format_import(*s.import) + ";";
            break;
        }
        case Stmt::Kind::Let: {
...
std::string format_decl(const Decl& decl, int indent) {
    ...
    switch (decl.get_kind()) {
        ...
        case Decl::Kind::Open: {
            const auto& d = static_cast<const OpenDecl&>(decl);
            result += "open " + format_import(*d.import) + ";";
            break;
        }
...
std::formatter<parsing::Type>::format(const parsing::Type& type, std::format_context& ctx) const {
    return std::formatter<std::string>::format(parsing::format_type(type), ctx);
}

std::format_context::iterator
std::formatter<parsing::Lit>::format(const parsing::Lit& lit, std::format_context& ctx) const {
    return std::formatter<std::string>::format(parsing::format_lit(lit), ctx);
}

std::format_context::iterator
std::formatter<parsing::Pat>::format(const parsing::Pat& pat, std::format_context& ctx) const {
    return std::formatter<std::string>::format(parsing::format_pat(pat), ctx);
}

std::format_context::iterator
std::formatter<parsing::Expr>::format(const parsing::Expr& expr, std::format_context& ctx) const {
    return std::formatter<std::string>::format(parsing::format_expr(expr), ctx);
}

std::format_context::iterator
std::formatter<parsing::Stmt>::format(const parsing::Stmt& stmt, std::format_context& ctx) const {
    return std::formatter<std::string>::format(parsing::format_stmt(stmt, 0), ctx);
}

std::format_context::iterator std::formatter<parsing::Import>::format(
    const parsing::Import& import,
    std::format_context& ctx
) const {
    return std::formatter<std::string>::format(parsing::format_import(import), ctx);
}

std::format_context::iterator
std::formatter<parsing::Cond>::format(const parsing::Cond& cond, std::format_context& ctx) const {
    return std::formatter<std::string>::format(parsing::format_cond(cond), ctx);
}

std::format_context::iterator std::formatter<parsing::Clause>::format(
    const parsing::Clause& clause,
    std::format_context& ctx
) const {
    return std::formatter<std::string>::format(parsing::format_clause(clause, 0), ctx);
}

std::format_context::iterator
std::formatter<parsing::Decl>::format(const parsing::Decl& decl, std::format_context& ctx) const {
    return std::formatter<std::string>::format(parsing::format_decl(decl, 0), ctx);
}

std::format_context::iterator std::formatter<parsing::Package>::format(
    const parsing::Package& pkg,
    std::format_context& ctx
) const {
    return std::formatter<std::string>::format(parsing::format_package(pkg), ctx);
}
